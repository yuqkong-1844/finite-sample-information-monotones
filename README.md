...
\[
U\in\Delta_{n,m},\quad \operatorname{rank}(U)\le 1
...
\bigl(\forall U,\ \operatorname{rank}(U)\le1\Rightarrow F(U)=0\bigr),
\]
...
$\Delta_{n,m}$. If $F$ is $C^1$ on $\Omega$, satisfies one-sided DPI on
...
If a polynomial $P$ satisfies DPI and rank-one vanishing on
...
\[
2n\le \operatorname{totalDegree}(P).
\]
...
satisfies one-sided DPI. More explicitly, the formalization defines the
...
generic $n\times m$ matrix. If $P$ satisfies DPI and rank-one vanishing,
Lean proves
...
It also proves DPI for every channel in the convex hull of the
